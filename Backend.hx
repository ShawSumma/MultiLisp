...
class Vm {
    private var locals : Array<Array<Var>>;
    private var captures : Array<Var>;
    private var stack : Array<Dynamic>;
    private var globals : Map<String, Dynamic>;
    private var pl : Int;
    private var rets : Array<Int>;
    public var values : Array<Dynamic>;
    public var opcodes: Array<Opcode>;
    public function new() {
        this.locals = [[]];
        this.stack = [];
        this.captures = [];
    ...
    public function initglobals() {
        function b_print(state: Vm, args: Array<Dynamic>) : Dynamic{
            for (i in args) {
                Sys.print(i);
            }
            return null;
        }
        function b_println(state: Vm, args: Array<Dynamic>) : Dynamic{
            b_print(state, args);
            Sys.println("");
            return null;
        }
        function b_add(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 0;
            var ret : Float = 0;
        ...
        function b_sub(state: Vm, args: Array<Dynamic>) : Dynamic{
            if (args.length == 1) {
                return -args[0];
            }
            var pl = 1;
        ...
        function b_mul(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 0;
            var ret : Float = 1;
            while (pl < args.length) {
                ret *= args[pl];
        ...
        function b_div(state: Vm, args: Array<Dynamic>) : Dynamic{
            if (args.length == 1) {
                return 1/args[0];
            }
            var pl = 1;
        ...
        function b_lt(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 1;
        ...
        function b_gt(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 1;
            var fst : Float = args[0];
            while (pl < args.length) {
                if (fst > args[pl]) {
        ...
        function b_lte(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 1;
            var fst : Float = args[0];
            while (pl < args.length) {
                if (fst <= args[pl]) {
        ...
        function b_gte(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 1;
            var fst : Float = args[0];
            while (pl < args.length) {
                if (fst >= args[pl]) {
        ...
        function b_eq(state: Vm, args: Array<Dynamic>) : Dynamic{
            var pl = 1;
            var fst : Float = args[0];
            while (pl < args.length) {
                if (fst == args[pl]) {
        ...
        function b_neq(state: Vm, args: Array<Dynamic>) : Dynamic{
        ...
        function b_time(state: Vm, args: Array<Dynamic>) : Dynamic{
            return Sys.time();
        }
        function b_timer(state: Vm, args: Array<Dynamic>) : Dynamic{
            var otime = Sys.time();
            return new Callable(function(state: Vm, args: Array<Dynamic>) : Dynamic{
        ...
        globals['*'] = new Callable(b_mul);
        globals['/'] = new Callable(b_div);
        ...
        for (line in lines) {
            if (line != '') {
                ...
                if (opcode == 'init') {
                    ...
                    opcodes.push(new Opcode('push', values.length));
                    ...
                    opcodes.push(new Opcode('arg', line.substring(16) == 'True' ? 1 : 0));
                }
                else {
                    if (line.length == 16) {
                        opcodes.push(new Opcode(opcode));
                    }
                    else {
                        opcodes.push(new Opcode(opcode, Std.parseInt(line.substring(16))));
                    }
                }
            }
        }
    ...
    public function runcode() {
        pl = 0;
        while (pl < opcodes.length) {
            var cur : Opcode = opcodes[pl];
            switch (cur.op) {
                case LOAD:
